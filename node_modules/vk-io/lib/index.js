 ...
 * @type {string}
 */
const DESKTOP_USER_AGENT = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/74.0.3729.169 Safari/537.36';
...
const { inspect: inspect$2 } = nodeUtil__default;
...
const { inspect: inspect$3 } = nodeUtil__default;
...
const { inspect: inspect$4 } = nodeUtil__default;
...
const { inspect: inspect$5, deprecate } = nodeUtil__default;
...
const { inspect: inspect$6 } = nodeUtil__default;
...
const { inspect: inspect$7 } = nodeUtil__default;
...
const { inspect: inspect$8 } = nodeUtil__default;
...
const { inspect: inspect$9 } = nodeUtil__default;
...
const { inspect: inspect$a } = nodeUtil__default;
...
const { inspect: inspect$b } = nodeUtil__default;
...
const { inspect: inspect$c, promisify: promisify$2 } = nodeUtil__default;
...
class Updates {
	...
	async startWebhook(
		...
		try {
			...
			this.webhookServer = tls
				? nodeHttps__default.createServer(tls, callback)
	...
	reloadMiddleware() {
		...
		if (this.hearStack.length !== 0) {
			stack.push(
				middlewareIo.getOptionalMiddleware(
...
const { inspect: inspect$d, promisify: promisify$3 } = nodeUtil__default;
...
class VK {
    ...
    [nodeUtil.inspect.custom](depth, options) {
        ...
        return `${options.stylize(name, 'special')} ${nodeUtil.inspect(payload, options)}`;
    }
}
